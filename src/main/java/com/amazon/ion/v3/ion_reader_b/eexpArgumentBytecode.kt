...
private val TX_ANNOTATION_1_SID = ToBytecodeTransformer { dest, _,  src, pos, _ ->
    ...
    dest.add(MacroBytecode.OP_ANNOTATION_SID.opToInstruction(sid))
    lengthOfSid
}
private val TX_ANNOTATION_2_SID = ToBytecodeTransformer { dest,  _, src, pos, _ ->
    ...
    dest.add(MacroBytecode.OP_ANNOTATION_SID.opToInstruction(sid1))
    p += lengthOfSid1
    // Second Annotation
    val lengthOfSid2 = IntHelper.lengthOfFlexUIntAt(src, p)
    val sid2 = IntHelper.readFlexUIntWithLengthAt(src, p, lengthOfSid2)
    dest.add(MacroBytecode.OP_ANNOTATION_SID.opToInstruction(sid2))
    p += lengthOfSid2
...
private val TX_ANNOTATION_1_FLEXSYM = ToBytecodeTransformer { dest,  _, src, pos, _ ->
    ...
    val remainingLength = if (flexsym < 0) {
        ...
        dest.add(MacroBytecode.OP_ANNOTATION_SYSTEM_SID.opToInstruction(sid - 0x60))
        1
    } else {
        dest.add(MacroBytecode.OP_ANNOTATION_SID.opToInstruction(flexsym))
        0
    }
