...
internal class IonManagedWriter_1_1(
    ...
    private fun writeMacroDefinition(name: String?, macro: TemplateMacro) {
        writeSystemSexp {
            ...
            //       memoize-ing this in the macro definition, or replacing it with a list of precomputed
            //       step-out indices.
    ...
    private inline fun handleSymbolToken(sid: Int, text: String?, kind: SymbolKind, rawWriter: IonRawWriter_1_1, preserveEncoding: Boolean = false) {
        // println("handleSymbolToken($sid, $text, $kind, rawWriter, $preserveEncoding)")
        // if (text != null) println("options.shouldWriteInline($kind, $text) = ${options.shouldWriteInline(kind, text)}")
        if (text == null) {
    ...
    private fun transferScalarOrStepIn(reader: IonReader, currentType: IonType) {
        ...
        if (reader.isNullValue) {
            ...
            // TODO: See if we can preserve the encoding of containers (usingLengthPrefix vs length-prefixed)
            IonType.LIST -> {
