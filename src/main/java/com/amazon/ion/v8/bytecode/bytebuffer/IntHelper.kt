...
object IntHelper {
    ...
    @JvmStatic
    private fun ByteBuffer.getInt(position: Int): Int {
    ...
    @JvmStatic
    private fun ByteBuffer.getLong(position: Int): Long {
    ...
    @JvmStatic
    fun readFixedUIntAt(source: ByteBuffer, start: Int, length: Int): Long {
        if (source.limit() < start + length) throw IonException("Incomplete data: start=$start, length=$length, limit=${source.limit()}")
        if (length > 4) {
            // TODO: See if we can simplify some of the calculations
    ...
    @JvmStatic
    fun readFlexIntLongValue(source: ByteBuffer, position: Int): Long {
        val firstByte = source.get(position)
        val numBytes = firstByte.countTrailingZeroBits() + 1
        val value = when (numBytes) {
            1 -> {
    ...
    @JvmStatic
    fun readFlexIntBigIntegerValue(source: ByteBuffer, position: Int): BigInteger {
        // FIXME
        var p = position
