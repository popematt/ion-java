...
class ByteArrayLabeledBytecodeGenerator(
    private val source: ByteArray,
    // TODO: add options to label only TLVs, label values that come from macros, etc.
): BytecodeGenerator {
    ...
    companion object {
        ...
        fun compileTopLevel(
            ...
            while (p < end && (opcode or 0x7 != 0XE7)) {
                dest.add2(Bytecode.LABEL_OFFSET.opToInstruction(), p)
            ...
            while (true) {
                ...
                when (op) {
                    ...
                    Bytecode.OP_OPT_PLACEHOLDER -> {
                        ...
                        val substitutionSize = dest.size() - 2 /* For label */ - destSize
...
//       Instead, we'll have a default value of "nothing".
        ...
        @OptIn(ExperimentalStdlibApi::class)
        private val OP_INVALID_TAGLESS = WriteBytecode { src, pos, op, dest, cp, macSrc, macIdx, symTab -> throw IonException("Encountered invalid opcode for tagless type: ${op.toHexString()}") }
