...
class StreamReaderImpl internal constructor(
    ...
    init {
        ...
        availableModules["_"] = ModuleReader.Module("_", SystemSymbols_1_1.allSymbolTexts().toMutableList(), mutableListOf())
        availableModules["\$ion"] = ModuleReader.Module("\$ion", SystemSymbols_1_1.allSymbolTexts().toMutableList(), mutableListOf())
    }
        ...
        when (presence) {
            ...
            2 -> {
                ...
                val symbolsList = if (length == 0) {
                    pool.getDelimitedList(start, source.limit() - start, symbolTable, this)
                } else {
                    source.position(start + length)
                    pool.getList(start, length, symbolTable)
                }
