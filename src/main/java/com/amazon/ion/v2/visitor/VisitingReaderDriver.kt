...
class VisitingReaderDriver {

    companion object {
        // TODO: Add static methods here, which will transparently create a driver instance, if necessary.
    }

    fun readAll(reader: ValueReader, visitor: VisitingReaderCallback) {
        while (readValue(reader, visitor)) {}
    }

    fun read(reader: ValueReader, visitor: VisitingReaderCallback) {
...
//        if (debug) {
//            if (reader is ValueReaderBase) {
//                println("Found ${TokenTypeConst(token)} in ${reader::class.simpleName} at ${reader.source}")
//            } else {
//                println("Found ${TokenTypeConst(token)}")
//            }
//        }
        when (token) {
            TokenTypeConst.IVM -> {
            ...
            TokenTypeConst.SEXP -> reader.sexpValue().use { r ->
                visitor.onSexp()?.let { sexpVisitor ->
                    readAll(r, sexpVisitor)
                    visitor.onSexpEnd()
                }
            }
            TokenTypeConst.STRUCT -> reader.structValue().use { r ->
                visitor.onStruct()?.let { structVisitor ->
                    while (readStructField(r, structVisitor)) {}
                    visitor.onStructEnd()
                }
            }
